...
def generate_task_description(style_1, style_2,
                              allow_content_change=True, lang='Japanese'):
    task_descr = [
        ...
        f"Without changing the content and the order of the text, please rewrite the article line by line in {lang} with {style_1} and {style_2} style of speaking, respectively."
        ]
...
def rewrite(config, sentences, args):
    task_descr = generate_task_description(style_1=config['keywords'][0], # ex: 'polite/formal (フォーマル)'
                                           ...
                                           allow_content_change=config['allow_content_change'], lang=args.language)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--input_fn', default='./data/shimogamo.txt')
    parser.add_argument('-o', '--output_fn', default='./data/shimogamo_4_styles.json')
